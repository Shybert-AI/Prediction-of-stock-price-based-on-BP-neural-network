...
def init_weights(m):  #m为模块
    ...
    method =  "kaiming_normal_"  #"normal_"
    if classname.find('Linear') != -1:
        if method =="kaiming_normal_":     #恺明正态分布初始化
            m.weight.data.zero_()  #先将定义线性层时的权重置为零
            nn.init.kaiming_normal_(m.weight, mode='fan_in')    #loss 4.2889
            #nn.init.kaiming_uniform_(m.weight,a=math.sqrt(5)) #均匀分布 #loss=4.2862
            m.bias.data.zero_()    
        else:   
                    m.weight.data.zero_()  #先将定义线性层时的权重置为零
...
## 模型
##第一次神经元的数目
#hidden_size1 = list(range(5,14))+[20,50,100]
##第二层神经元的数目
#hidden_size2 = list(range(5,14))+[20,50,100]
#MAE = {}
#for i in hidden_size1:
    #for j in hidden_size2:
        #model = NeuralNet(15,i,j, 1,p=0,active_func="Relu").to(device)
        ## 梯度下降优化器和学习率变化策略
...
pf = pd.read_json("result.json")

re = pf.describe()
re = re.apply(lambda x:round(x,4))
re.to_csv("神经元数目效果.csv")
